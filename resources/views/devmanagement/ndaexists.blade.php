                    ...
                    <div class="divDContent DDC">
                        <div class="DDCNDA">
                            ...
                            <form action="/tests/{{$test->id}}/nda" method="post">
                                ...
                                <div class="DDCNDAText"><label class="labelMain">Non-Disclosure Agreement (NDA)</label><textarea class="inputMain"name="nda">{{$nda[0]->nda}}</textarea></div>
                                ...
                                <div class="divNDATimeLimit"><label class="labelMain">Recall time-limit</label>
                                    <p class="paraNDATL">The test invite will be unsent if the tester rejects the NDA, or if they don't respond after the specified time limit.</p><select class="custom-select inputMain" name="timelimit">
                                        <optgroup label="Game Test NDA Time Limit" class="optgroupIDANGF">
                                            <option value="NULL" @if($nda[0]->timelimit==0)selected @endif>No limit</option>
                                            <option value="1" @if($nda[0]->timelimit==1)selected @endif>24 Hours</option>
                                            <option value="14" @if($nda[0]->timelimit==14)selected @endif>14 Days</option>
                                            <option value="30" @if($nda[0]->timelimit==30)selected @endif>30 Days</option>
                                            <option value="90" @if($nda[0]->timelimit==99)selected @endif>90 Days</option>
                                            <option value="365" @if($nda[0]->timelimit==365)selected @endif>365 Days</option>
                                        </optgroup>
                                    </select>
                                </div>
